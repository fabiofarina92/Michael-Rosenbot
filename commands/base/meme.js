...
module.exports = {
  ...
  execute(config, message, args) {
    ...
    if (args.length > 0) {
      if (args[0].toUpperCase() === "RECENT") {
        message.channel.messages.fetch({ limit: 3 }).then((messages) => {
          topText = messages.array()[2].content;
          bottomText = messages.array()[1].content;
          selectedMeme = shuffle(memeList)[0];
          message.channel.send(`http://apimeme.com/meme?meme=${selectedMeme}&top=${topText.replaceAll(" ", "+")}&bottom=${bottomText.replaceAll(" ", "+")}`);
        });
        return;
      }
      if (args[0].includes(";")) {
        let argsString = args.join(" ").split(";");
        topText = argsString[0].replaceAll(" ", "+");
        bottomText = argsString[1].replaceAll(" ", "+") || "bottom+text";
        selectedMeme = shuffle(memeList)[0];
      } else {
        if (memeList.includes(args[0])) {
          selectedMeme = args[0];
        } else {
          selectedMeme = shuffle(memeList)[0];
        }
        if (args[1] && args.slice(1).join().includes(";")) {
          let argsString = args.slice(1).join(" ").split(";");
          topText = argsString[0].replaceAll(" ", "+");
          bottomText = argsString[1].replaceAll(" ", "+") || "bottom+text";
        }
      }
    ...
    if (topText === null) {
      getText(message, "What would you like the top text to be?").then((query1) => {
        topText = query1;
        getText(message, "What would you like the bottom text to be?").then((query2) => {
          bottomText = query2;
          selectedMeme = shuffle(memeList)[0];
          message.channel.send(`http://apimeme.com/meme?meme=${selectedMeme}&top=${topText}&bottom=${bottomText}`);
        });
      });
    } else {
      message.channel.send(`http://apimeme.com/meme?meme=${selectedMeme}&top=${topText}&bottom=${bottomText}`);
    ...
    // }
    // if (args.length > 1) {
    //   let argString = args.slice(1).join(" ").split(";");
    //   let topText = argString[0];
    //   let bottomText = argString[1] || "bottom text";
    //   message.channel.send(`http://apimeme.com/meme?meme=${selectedMeme}&top=${topText}&bottom=${bottomText}`);
    // } else {
    //   getText(message, "What would you like the top text to be?").then((topText) => {
    //     getText(message, "What would you like the bottom text to be?").then((bottomText) => {
    //       message.channel.send(`http://apimeme.com/meme?meme=${selectedMeme}&top=${topText}&bottom=${bottomText}`);
    //     });
    //   });
    // }
...
const getText = (message, query) => {
  return new Promise((resolve, reject) => {
    ...
    collector.on("collect", (response) => {
      resolve(response.content.replaceAll(" ", "+"));
    });
  });
};
