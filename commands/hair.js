...
module.exports = {
  ...
  execute(config, message, args) {
    ...
    httpRequestsHelper
      .sendBasicGet(
        `https://api.fresha.com/locations/348940/timeslot-availability?offer-item-ids=sv%3A5163833&date-from=${NOW}&date-to=${EIGHT_WEEKS_FROM_NOW}`
      )
      .then((response) => {
        ...
        if (foundAppointment) {
          let formattedMessage = `Found appointment for you on ${moment(
            foundAppointment.id
          ).format("dddd [the] Do MMM, YYYY")}`;
