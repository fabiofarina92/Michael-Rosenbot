...
client.on("messageReactionAdd", async (reaction, user) => {
  ...
  if (
    user.username === "prae" ||
    user.username === "jahoo" ||
    user.username === "rgrwco" ||
    user.username === "Dezine"
  ) {
...
client.on("message", (message) => {
  ...
  commandDB
    ...
    .forEach((cmd) => {
      if (
        message.content &&
        cmd.command.toUpperCase() === message.content.toUpperCase()
      ) {
  ...
  if (!message.content.match(configuration.PREFIXMATCH) || message.author.bot)
    return;
